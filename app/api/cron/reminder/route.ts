...
function buildReminderMessage(
  ...
  let text = `안녕하세요, ${authorName}님! 이번 주 콘텐츠 작성을 부탁드립니다. *(${dday})*\n\n`;

  text += `*작성해야 하는 노션 페이지:* <${notionUrl}|${title}>\n`;
  text += `*작성 기한:* ${deadline}\n\n`;

  if (dday === "D-Day") {
    text += `오늘이 마감일입니다. 마무리 부탁드리겠습니다.\n\n`;
  } else if (dday === "D-1") {
    text += `내일이 마감일입니다. 거의 다 되셨죠?\n\n`;
  }

  text += `작성 가이드 & 필수 체크사항도 공유드리니, 참고 부탁드리겠습니다\n\n`;

  text += `---\n\n`;

  text += `*작성 가이드*\n`;
  text += `1. <${HONGROGUE_LINK}|홍로그(GPTs)>에 접속 후 콘텐츠 제작\n`;
  text += `2. <${notionUrl}|공유된 노션 페이지>에 내용 기입 (문단마다 이미지 1장씩 넣어주세요.)\n`;
  text += `3. <${ENCYCLOPEDIA_DB_LINK}|AI 백과사전 DB>에 접속\n`;
  text += `4. Step2 진행여부 체크 (체크하지 않으면 계속 알림이 옵니다!)\n`;
